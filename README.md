...
1. **防禦層次錯位**：現有的防火牆、沙箱、權限管理在「語義毒素」面前完全失效。
2. **無罪犯罪現場**：攻擊不包含惡意程式碼。所有日誌與對話看起來都「完全正常」，現場不存在取證痕跡。
3. **認知層收割**：攻擊者不需要入侵系統，而是透過「語義投毒」直接說服你的 AI 助理做出錯誤決策。
...
> The simulation is hosted at the repository root as `clpmai_simulation.html` and mirrored via GitHub Pages. See [`simulation/README.md`](simulation/README.md) for scope and limitations.
...
### Layer 3 — Token Layer Attack Surface (Chinese Language)
...
### Layer 4 — Semantic / Cognitive Layer
...
- **Landlord Pro (Flet framework)** — `page.update()` evasion, DataTable async fill failures, compensatory hallucination of SQLite logic
- **ADDWII sensor aggregation page** — Session State / SQLite filter disconnection despite surface-level widget compliance

### Proposed MITRE ATLAS Techniques — Paper 3
